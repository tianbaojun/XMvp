...
public class EaseConversationAdapater extends ArrayAdapter<EMConversation> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (holder == null) {
            ...
            holder.unreadLabel = (TextView) convertView.findViewById(R.id.unread_msg_number);  // 头像右上角消息个数
            holder.message = (TextView) convertView.findViewById(R.id.message);   // 聊天内容
        ...
        if (conversation.getType() == EMConversationType.GroupChat) {
            ...
            EaseUserUtils.setUserNickNameAvater(username, holder.name);
            holder.motioned.setVisibility(View.GONE);
        }

        if (conversation.getUnreadMsgCount() > 0) {
            // show unread message count
            LogUtil.e("环信消息个数 = ",String.valueOf(conversation.getUnreadMsgCount()));
