...
import com.zjhz.teacher.hx.model.EaseAtMessageHelper;
import com.zjhz.teacher.hx.utils.EaseCommonUtils;
import com.zjhz.teacher.hx.utils.EaseUserUtils;
import com.zjhz.teacher.hx.widget.EaseAlertDialog;
import com.zjhz.teacher.hx.widget.EaseChatExtendMenu;
import com.zjhz.teacher.hx.widget.EaseChatInputMenu;
import com.zjhz.teacher.hx.widget.EaseChatMessageList;
import com.zjhz.teacher.hx.widget.EaseVoiceRecorderView;
import com.zjhz.teacher.hx.widget.chatrow.EaseCustomChatRowProvider;
...
//        titleBar.setTitle(activity.name);
        if (AppContext.getInstance().hxId != null && AppContext.getInstance().hxId.size() > 0) {
            for (int i = 0; i < AppContext.getInstance().hxId.size(); i++) {
                if (toChatUsername.equals(AppContext.getInstance().hxId.get(i).id)) {
                    titleBar.setTitle(AppContext.getInstance().hxId.get(i).name);
                }
            }
        }else{
                ...
                new Handler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        if (listView.getFirstVisiblePosition() == 0 && !isloading && haveMoreData) {
                            ...
                            ToastUtils.showShort(R.string.no_more_messages);
                        }
                        swipeRefreshLayout.setRefreshing(false);
                    }
                }, 600);
...
//                Toast.makeText(getActivity(), toastContent, Toast.LENGTH_SHORT).show();
    ...
    protected void sendTextMessage(String content) {
        if(EaseAtMessageHelper.get().containsAtUsername(content)){
            ...
            message.setAttribute("head", SharedPreferencesUtils.getSharePrefString(ConstantKey.PhotoUrlKey));
            message.setAttribute("name", SharedPreferencesUtils.getSharePrefString(ConstantKey.NickNameKey));
            sendMessage(message);
        }
    }
    ...
    protected void sendFileByUri(Uri uri){
        ...
        if (file.length() > 10 * 1024 * 1024) {
            ToastUtils.showShort( R.string.The_file_is_not_greater_than_10_m);
            return;
        }
    ...
    protected void selectPicFromCamera() {
        ...
        LogUtil.e(TAG,"PathUtil.getInstance().getImagePath() = " + Environment.getExternalStorageDirectory());
        LogUtil.e(TAG,"cameraFile = " + cameraFile);
        LogUtil.e(TAG,"cameraFile.getParentFile() = " + cameraFile.getParentFile());
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == REQUEST_CODE_CAMERA) { // capture new image  // 拍照后的图片
                if (cameraFile != null && cameraFile.exists())
                    sendImageMessage(cameraFile.getAbsolutePath());  // 发送图片
            } else if (requestCode == REQUEST_CODE_LOCAL) { // send local image 本地相册中获取到的图片
                if (data != null) {
    ...
    class GroupListener extends EaseGroupRemoveListener {
        ...
        public void onGroupDestroy(final String groupId, String groupName) {
            ...
            getActivity().runOnUiThread(new Runnable() {
                public void run() {
                    if (toChatUsername.equals(groupId)) {
                        ToastUtils.showShort(R.string.the_current_group);
                        getActivity().finish();
                    }
                }
            });
