...
//		Bitmap bitmap = b.copy(Bitmap.Config.ARGB_8888, true);
		Bitmap bitmap = CMCPhotoUtil.zoomImage(b,300,300);//压缩图片
		if (defaultWidth == 0) {
		...
		if (mBorderInsideColor != defaultColor
			...
			drawShadow(canvas, radius + mBorderThickness / 2); //阴影
	...
	/**画圆形阴影[失败]*/
	private void  drawShadow(Canvas canvas,int radius){
		drawCircleBorder(canvas,radius+10,0xFFE2E2E2);
		drawCircleBorder(canvas,radius+1,0xFFEFEFEF);
