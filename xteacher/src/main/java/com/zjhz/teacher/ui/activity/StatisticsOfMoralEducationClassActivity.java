 ...
 */
public class StatisticsOfMoralEducationClassActivity extends BaseActivity implements SwipeRefreshLayout.OnRefreshListener {
    private TextView titleBackImg;
    private TextView titleTv;
    private TextView right_tv;
    private ListView refreshListview;
    private RefreshLayout refreshLayout;
    private final static String TAG = StatisticsOfMoralEducationClassActivity.class.getSimpleName();
...
//        refreshListview.setAdapter(adapter);
    ...
    public void selectGrade() {
        if (!grades.isEmpty() && !clazzs.isEmpty()) {
            mOptionsGradeView.setPicker(grades, clazzs, index_grade, index_clazz, 0);
            ...
            mOptionsGradeView.setOnoptionsSelectListener(new OptionsGradeView.OnOptionsSelectListener() {
                ...
                public void onOptionsSelect(int options1, int option2, int options3) {
                    if (clazzs.get(options1).size() > 0 && option2 < clazzs.get(options1).size()) {
                        right_tv.setText(clazzs.get(options1).get(option2));
                        index_grade = options1;
                        index_clazz = option2;
                        LogUtil.e("index_grade = " + index_grade + " index_clazz = " + index_clazz);
                        String gradeId = clazzsGrades.get(index_grade).getGradeId();
                        String classId = clazzsGrades.get(index_grade).getDetail().get(index_clazz).getClassId();
