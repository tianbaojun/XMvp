 ...
 */
public class SectorMenu  extends ViewGroup implements View.OnClickListener  {
    ...
    public SectorMenu(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.FlyMenu);
        int position = ta.getInt(R.styleable.FlyMenu_position, RIGHT_BOTTOM);
        switch (position) {
            case LEFT_TOP:
...
//        mRadius = (int) ta.getDimension(R.styleable.FlyMenu_radius, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 10, context.getResources().getDisplayMetrics()));
        mRadius = (int) getResources().getDimension(R.dimen.location_main_expangd_radius);    // 半径
        ta.recycle();
    }
