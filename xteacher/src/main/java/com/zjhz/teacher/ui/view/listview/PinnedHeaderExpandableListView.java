package com.zjhz.teacher.ui.view.listview;
...
public class PinnedHeaderExpandableListView extends ExpandableListView implements AbsListView.OnScrollListener {
    ...
    private int previousGroupPos = -1;
    ...
    protected void refreshHeader() {
        ...
        if (firstVisiblePos < getHeaderViewsCount())
            firstVisibleGroupPos = getPackedPositionGroup(getExpandableListPosition(getHeaderViewsCount()));
        else
            firstVisibleGroupPos = getPackedPositionGroup(getExpandableListPosition(firstVisiblePos));
...
//                Log.w(TAG, "Warning : refreshHeader getChildAt(1)=null");
        ...
        if (group == 0 && firstVisiblePos < getHeaderViewsCount()) {
            mHeaderView.setVisibility(GONE);
        } else {
            mHeaderView.setVisibility(VISIBLE);
        }

        if (mHeaderUpdateListener != null && mHeaderView.getVisibility() == VISIBLE) {
            if (previousGroupPos != firstVisibleGroupPos) {
                mHeaderUpdateListener.updatePinnedHeader(mHeaderView, firstVisibleGroupPos);
                previousGroupPos = firstVisibleGroupPos;
            }
        }
